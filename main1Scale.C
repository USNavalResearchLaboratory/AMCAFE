...
int main(int argc, char *argv[])
{
  ...
  std::string filbaseTemp,filbaseOut,filout,neighOrder,neighType,filLogOut,filextScale;
  double mu; //2e11;//5e9 // rate for baseplate voronoi;
  double heightBase;
  // schwalbach parameters
  int patternID,numScale;
  ...
  if (argc==2){
    filextScale=argv[1];
  } else { std::cout << "provide # of processors as argument"<<std::endl;}
